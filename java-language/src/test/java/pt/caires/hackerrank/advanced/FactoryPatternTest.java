...
public class FactoryPatternTest {
    ...
    public static void main(final String[] args) {
        ...
        if (failures > 0) {
            throw new RuntimeException(
                    "Incurred " + failures + " failures while testing " + FactoryPattern.class.getSimpleName());
        } else {
    ...
    private static int checkResult(final Food food, final Class expected) {
        ...
        if (food != null && food.getClass() != expected) {
            System.err.println(
                    "Expected a Food of type " + expected.getSimpleName() + ", got a Food of type " + food.getClass()
                            .getSimpleName());
            return 1;
        }
