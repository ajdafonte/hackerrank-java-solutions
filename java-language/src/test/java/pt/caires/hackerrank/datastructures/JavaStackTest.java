...
public class JavaStackTest {
    ...
    public static void main(final String[] args) {
        ...
        if (failures > 0) {
            throw new RuntimeException(
                    "Incurred " + failures + " failures while testing " + JavaStack.class.getSimpleName());
        } else {
    ...
    private static int executeTest(final String input, final Boolean expectedResult) {
        ...
        if (!result.equals(expectedResult)) {
            System.err.println("For input '" + input + "', got the following result '" + result + "'. Expected result '"
                                       + expectedResult + "'.");
            return 1;
        }
